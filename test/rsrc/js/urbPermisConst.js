$(function() {
	...
	save=function(resId,posId,pcId){
		...
		if($("#pc_sise").val().length < 2){
			alert ("Sise Obligatoire !");
			$("#pc_sise").focus();
			return (0);
		}
		...
		$.ajax({
			   ...
			   success: function(data){
				   location.href = "!urb!index!urbPermisConst!index!"+resId+"!"+posId+"!"+data;
			   },
...
addDocs=function(resId,posId,pcId){
	if(pcId == ''){
			...
			"Enregistrer": function(){ 
				...
				$.ajax({
					...
					success: function(data){
					 ...
					 location.href = "!urb!index!urbPermisConst!index!"+resId+"!"+posId+"!"+pcId;
					 },
					   error:function(xhr, ajaxOptions, thrownError){
...
printPc=function(pid){
	...
	location.href = "urbPdf!index!permisConst!" + pid;
}


printRecepisse=function(pc_id,pc_recepisse){
	if(pc_id == ''){
		alert("Rien a imprimer, Aucun Permis de Construire Sélectionné !");
		return;
	}
	if(pc_recepisse == ''){
		alert("Rien a imprimer, Aucun Numéro de Récépissé !");
		return;
	}
	location.href = "urbPdf!index!permisConstRecepisse!" + pc_id ;
}
...
delDoc=function(resId,posId,pcId,iddoc){
	...
	openRural = function(resId,posId,rurid,pcId,pc_resultat_com){
		/* 
 		//  On peut déposer un dossier d'aide rural sans permis de construction
		if(!((pc_resultat_com == '1')||(pc_resultat_com == '2'))){
			alert('Dossier Non Favorable !');
			return;
		}
		
		$.ajax({
			   type: "POST", 
			   url: "!urbPermisConst!CheckOpenRural", 
			   data: {
				   pcId : pcId
			   },
			   async: false, 
		...
		*/ 
		...
		location.href = "!urb!index!urbRural!index!"+resId+"!"+posId+"!"+rurid+"!"+pcId;
	}
