$(function() {
		...
		if(confirm("Voulez vous supprimer cette catégorie ?")){
			$.ajax({
				   ...
				   success: function(data){
					   alert(data);
					   refreshCat();
					   //location.href = "four!index!fourProd!"+fur_id;
				   },
	...
	delProd=function(prd_id,fur_id){
	
		if(confirm("Voulez vous supprimer ce produit ?")){
			$.ajax({
				   type: "POST", 
				   url: "!fourProd!DelProd", 
				   data: {
					   prd_id : prd_id
				   },
				   async: false, 
	...
	saveCat=function( cat_id,fur_id,cat_nom="",cat_description=""){
		...
		$("#addProdDialog").dialog({
			...
			buttons: {
				"Enregistrer": function(){ 
					...
					if($("#prd_ref").val().length < 1){
						alert ("Référence Produit obligatoire  !");
						return (0);
					}
					...
					if($("#prd_tva").val().length < 2){
						alert ("TVA  obligatoire  !");
						return (0);
					}
						...
						success: function(data){
						 if(data.length >0){
							 alert(data);
						   	...
						   	//location.href = "four!index!fourProd!"+redId;
		...
		$.each(list, function(key, row) {
			...
			$("#tableCat").append(
					"<tr id=\"crow"+row['cat_id']+"\" onclick=\"selectCat('"+row['cat_id']+"'); refreshProd('"+row['cat_id']+"')\" >" +
						...
						"<td style=\"width: 70px\">"+
						 	"<button type=\"button\" class=\"btn btn-danger btn-xs\" onclick=\"saveCat('"+row['cat_id']+"','"+four_id+"','"+row['cat_nom']+"','"+row['cat_description']+"')\"><span class=\"glyphicon glyphicon-edit\" aria-hidden=\"true\"></span> </button>" +
						 	"&nbsp;"+
						 	"<button type=\"button\" class=\"btn btn-danger btn-xs\" onclick=\"delCat('"+row['cat_id']+"','"+four_id+"')\"><span class=\"glyphicon glyphicon-trash\" aria-hidden=\"true\"></span> </button>" +
						"</td>"+
					"</tr>"
			);
